use kaleidoscope_ast::{
    node::{reify_expr_node, reify_expr_node_ref},
    nodes::{
        BinaryOperatorNode,
        FloatNode,
        IdentifierNode,
...
fn test_parse_integer() {
    ...
    let expression = parser
        .parse_integer_expression(ltuplemut!(&mut stream, &mut tokenizer))
        .unwrap()
        .unwrap();
...
fn test_parse_float() {
    ...
    let expression = parser
        .parse_float_expression(ltuplemut!(&mut stream, &mut tokenizer))
        .unwrap()
        .unwrap();
...
fn test_parse_variable_expression() {
    ...
    let expression = parser
        .parse_variable_expression(ltuplemut!(&mut stream, &mut tokenizer))
        .unwrap()
        .unwrap();
...
fn test_round_bracket() {
    ...
    let expression = parser
        .parse_round_bracket_expression(ltuplemut!(&mut stream, &mut tokenizer))
        .unwrap()
        .unwrap();
    let node = reify_expr_node::<FloatNode>(expression).unwrap();
...
#[test]
fn test_binop_0() {
    let (mut parser, mut stream, mut tokenizer) = get_parser("1 + 2");
    let expression = parser
    ...
    let left = reify_expr_node_ref::<IntegerNode>(node.get_first()).unwrap();
...
#[test]
fn test_binop_1() {
    let (mut parser, mut stream, mut tokenizer) = get_parser("1 + 2 * 3");
    let expression = parser
        .parse_expression(ltuplemut!(&mut stream, &mut tokenizer))
        .unwrap()
        .unwrap();
    ...
    let left = reify_expr_node_ref::<IntegerNode>(node.get_first()).unwrap();
    assert_eq!(left.get_value(), 1);
    let right_node = reify_expr_node_ref::<BinaryOperatorNode>(node.get_second()).unwrap();
    assert_eq!(*right_node.get_operator(), Operator::Multiply);
    let right_left = reify_expr_node_ref::<IntegerNode>(right_node.get_first()).unwrap();
    assert_eq!(right_left.get_value(), 2);
    let right_right = reify_expr_node_ref::<IntegerNode>(right_node.get_second()).unwrap();
    assert_eq!(right_right.get_value(), 3);
}

#[test]
fn test_binop_2() {
    let (mut parser, mut stream, mut tokenizer) = get_parser("1 + 2 * 3 / 4");
    let expression = parser
        .parse_expression(ltuplemut!(&mut stream, &mut tokenizer))
        .unwrap()
        .unwrap();
    ...
    let right_left_node =
        reify_expr_node_ref::<BinaryOperatorNode>(right_node.get_first()).unwrap();
    assert_eq!(*right_left_node.get_operator(), Operator::Multiply);
    let right_left_left = reify_expr_node_ref::<IntegerNode>(right_left_node.get_first()).unwrap();
    assert_eq!(right_left_left.get_value(), 2);
    let right_left_right =
        reify_expr_node_ref::<IntegerNode>(right_left_node.get_second()).unwrap();
...
#[test]
fn test_binop_3() {
    let (mut parser, mut stream, mut tokenizer) = get_parser("1 + 2 * 3 / 4 - 5");
    let expression = parser
        .parse_expression(ltuplemut!(&mut stream, &mut tokenizer))
        .unwrap()
        .unwrap();
    ...
    let node = reify_expr_node::<BinaryOperatorNode>(expression).unwrap();
    let left_node = reify_expr_node_ref::<BinaryOperatorNode>(node.get_first()).unwrap();
    assert_eq!(*left_node.get_operator(), Operator::Plus);
    let left_left = reify_expr_node_ref::<IntegerNode>(left_node.get_first()).unwrap();
    assert_eq!(left_left.get_value(), 1);
    let left_right_node =
        reify_expr_node_ref::<BinaryOperatorNode>(left_node.get_second()).unwrap();
    assert_eq!(*left_right_node.get_operator(), Operator::Divide);
    let left_right_left_node =
        reify_expr_node_ref::<BinaryOperatorNode>(left_right_node.get_first()).unwrap();
    assert_eq!(*left_right_left_node.get_operator(), Operator::Multiply);
    let left_right_left_left =
        reify_expr_node_ref::<IntegerNode>(left_right_left_node.get_first()).unwrap();
    assert_eq!(left_right_left_left.get_value(), 2);
    let left_right_left_right =
        reify_expr_node_ref::<IntegerNode>(left_right_left_node.get_second()).unwrap();
    assert_eq!(left_right_left_right.get_value(), 3);
    let left_right_right =
        reify_expr_node_ref::<IntegerNode>(left_right_node.get_second()).unwrap();
