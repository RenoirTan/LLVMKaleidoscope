...
/// Represents the location of a byte in a file or a stream.
/// This index allows you to store the line on which the byte sits
/// (i.e. how many newlines have passed) if you have that data and the
/// column of the byte (i.e. how many bytes there are before it).
/// If the byte is the first of many in a line, then its column will be 0.
/// 
/// If you don't know how many lines have passed, or if that data is
/// irrelevant, you can set `FileIndex.line` to `None` in `FileIndex::new`.
/// 
...
impl FileIndex {
    ...
    /// If the encoding of the file you are reading is UTF-16, these 2 bytes
    /// become 4 bytes, as each ASCII character requires 2 bytes to be
    /// encoded in UTF-16.
    /// 
    /// # Example
    /// 
    /// ```
    /// let fi_1 = FileIndex::new(None, 5); // 5 bytes have passed
    /// // OwO what's this? A new line?
    /// let fi_2 = fi_1.newline('\r\n'.len()); // 2 bytes in UTF-8
...
impl Add<usize> for FileIndex {
    ...
    fn add(self, rhs: usize) -> Self::Output {
        Self {line: self.get_line(), column: self.get_column() + rhs}
    }
}
